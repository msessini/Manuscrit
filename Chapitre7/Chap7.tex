...
La MET utilisée est reconstruite par l'algorithme PUPPI, choisi pour sa meilleure performance face à l'empilement au lieu de la MET issue du flux de particules (PF MET) \cite{puppi}. Les corrections d'énergie des jets sont ensuite propagées à la MET de la façon suivante :
...
La classification des évènements est produite par un arbre de décision boosté multi-classe entraîné avec la librairie \textsc{XGBoost} \cite{XGboost}. Les évènements sont ainsi classifiés selon trois catégories : \\
